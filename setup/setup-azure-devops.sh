...
az pipelines variable-group create \
  ...
  --variables environmentId=DEV azureSubscription=$ADO_SC_NAME hostPoolName=$DEV_HPOOL_NAME resourceGroup=$DEV_RG storageAccount=$AVD_STRG_NAME msixAzureVMLocalPath='c:\avdmsix' msixAppAttachUNCServer=fileshare.domain.onmicrosoft.com msixAppAttachShareName=avdmsix \
  --authorize true \
